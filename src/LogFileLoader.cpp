...
LogFileLoader::LogFileLoader(std::string fname) { filename = fname; }

void LogFileLoader::setFilename(std::string f) { filename = f; }

int LogFileLoader::getRotation(const int &idx) const { return rotation[idx]; }
...
std::vector<double> LogFileLoader::getX() const { return x_translation; }

std::vector<double> LogFileLoader::getZ() const { return z_translation; }

std::vector<double> LogFileLoader::getTheta() const { return rotation_in_rads; }
...
double LogFileLoader::getTime(const int &idx) const { return times[idx]; }

int LogFileLoader::getTriggerIndex() const { return trigger_index; }

bool LogFileLoader::containsAcquisition() const { return hasAcquisition; }

ptime LogFileLoader::getTriggerTime() const { return trigger_ptime; }

std::vector<ptime> LogFileLoader::getPTimes() const { return ptimes; }

std::vector<double> LogFileLoader::getTimes() const { return times; }
...
int LogFileLoader::findIndexOfNearestDuration(double this_duration) const {
  return 1 + closest(times,
                     this_duration); // a vector of double - see
                                     // calculateDurationsAndRotations() below
}
...
bool LogFileLoader::calculateDurationsAndRotations() {
  if (!hasAcquisition) {
    ...
    std::cout << "Calculating rotations and times from log file data..."
              << std::endl;
    auto first_time = getTriggerTime();
    unsigned int count = 0;
    for (std::vector<ptime>::iterator i = ptimes.begin(); i != ptimes.end();
         ++i) {
      auto duration = *i - first_time;
      times.push_back(FpMilliseconds(duration).count() / 1000.0);
      auto raw_rotation =
          2 * M_PI *
          (double(rotation[count]) / (double)rotary_encoder_units_per_turn);
      rotation_in_rads.push_back(constrainAngleToPi(raw_rotation));
      ++count;
    }
    std::cout << "Finished calculating rotations and times." << std::endl;
    std::cout << "The raw log file has " << times.size() << " timestamps in it."
              << std::endl;
...
bool LogFileLoader::interpTiffData(std::vector<double> tiffTimestamps) {
  if (!(times.empty())) {
    std::cout << "\nInterpolating logfile data from .tiff file timestamps..."
              << std::endl;
    std::vector<double>::iterator low;
    std::vector<double> interpRotations;
    for (std::vector<double>::iterator i = tiffTimestamps.begin();
         i != tiffTimestamps.end(); ++i) {
...
void LogFileLoader::setTriggerIndex(int n) { trigger_index = n; }

std::vector<std::pair<int, int>>
LogFileLoader::findStableFrames(const unsigned int minFrames,
  ...
  // set a small tolerance for rounding error and return zeros where there's no
  // rotation, 1 otherwise
  std::transform(theta_df.begin(), theta_df.end(), theta_df.begin(),
                 [minAngle](double &n) {
                   if (n > -minAngle && n < minAngle)
                     return 0;
                   else
                     return 1;
                 });
